...
class ngenDisplayResource:
	def __init__( self ):
		...
		self.wScale = 1.0
		self.hScale = 1.0
		self.formatConvert = {	"R8G8B8A8" : 1, "sRGB_R8G8B8A8" : 2, "R16G16B16A16" : 3, "R11G11B10F" : 4, "D24S8" : 5 }
	...
	def loadJson( self, stringTable, jsonData ):
		...
		if self.formatConvert[ jsonData[ "format" ] ]:
			self.format = self.formatConvert( jsonData[ "format" ] )
		else:
...
class ngenClearInfo:
	...
	def export( self, binaryWriter ):
		binaryWriter.writeBool32( self.clearColor )
		binaryWriter.writeBool32( self.clearDepth )
		binaryWriter.writeUInt32( self.stencilValue )
		binaryWriter.writeFloat32( self.colorValue )
		binaryWriter.writeFloat32( self.color[ 0 ] )
		binaryWriter.writeFloat32( self.color[ 1 ] )
		binaryWriter.writeFloat32( self.color[ 2 ] )
		binaryWriter.writeFloat32( self.color[ 3 ] )
...
class ngenGenerator:
	...
	def loadJson( self, strinyTable, jsonData ):
		self.name = stringTable.addString( jsonData[ "name" ] )
